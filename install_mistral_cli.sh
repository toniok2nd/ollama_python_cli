#!/usr/bin/env bash
# ---------------------------------------------------------------
# install_mistral_cli.sh
# Installs the Python CLI from https://github.com/toniok2nd/ollama_python_cli
# and makes it available as a global Bash command called `mymistral`.
#
# Tested on:
#   - Ubuntu 20.04 / 22.04
#   - macOS (with Homebrew)
...
# ---------------------------------------------------------------
REQ_FILE="requirements_mistral.txt"

if [ -f "${REPO_BASE}/${REQ_FILE}" ]; then
    log "Installing Mistral CLI dependencies using ${REQ_FILE} ..."
    pip install --upgrade pip setuptools wheel
    pip install -r "${REPO_BASE}/${REQ_FILE}"
else
    log "Warning: ${REQ_FILE} not found. Falling back to core packages..."
    pip install --upgrade pip setuptools wheel
    pip install rich prompt_toolkit pyperclip mistralai mcp argcomplete
fi

# ---------------------------------------------------------------
# 6. Install the command wrapper
# ---------------------------------------------------------------
# The actual script you want to expose is called cliMistral.py
CLI_SCRIPT="${REPO_BASE}/cliMistral.py"

if [ ! -f "$CLI_SCRIPT" ]; then
    error "Could not locate cliMistral.py at expected path: $CLI_SCRIPT"
fi

WRAPPER_PATH="${WRAPPER_DIR}/mymistral"

# Create a tiny wrapper that forwards to the virtualenv python interpreter
cat >"$WRAPPER_PATH" <<'EOF'
#!/usr/bin/env bash
# PYTHON_ARGCOMPLETE_OK
# Wrapper for mistral_python_cli → cliMistral.py
...
if PATH="${WRAPPER_DIR}:$PATH" "${VENV_DIR}/bin/python" -m argcomplete.scripts.register_python_argcomplete mymistral > "$COMPLETION_FILE" 2>/dev/null; then
...
complete -o nospace -o default -o bashdefault -F _python_argcomplete mymistral
EOF
...
# ---------------------------------------------------------------
if [[ ":$PATH:" != *":${WRAPPER_DIR}:"* ]] || ! grep -q "mymistral_completion.bash" "${HOME}/.bashrc" 2>/dev/null; then
    if [ -w "${HOME}/.bashrc" ]; then
        if [[ ":$PATH:" != *":${WRAPPER_DIR}:"* ]]; then
            echo '' >> "${HOME}/.bashrc"
            echo "# Added by mistral_python_cli installer" >> "${HOME}/.bashrc"
            echo "export PATH=\"${WRAPPER_DIR}:\$PATH\"" >> "${HOME}/.bashrc"
            log "Appended PATH export to ${HOME}/.bashrc"
        fi
        
        if ! grep -q "mymistral_completion.bash" "${HOME}/.bashrc" 2>/dev/null; then
...
# ---------------------------------------------------------------
# 9. Remind user to configure Mistral API key
# ---------------------------------------------------------------
echo ""
log "🎉 Installation complete!"
echo ""
echo "To start using the Mistral CLI:"
echo "  1. Run: mymistral --config-mistral"
echo "     (or set MISTRAL_API_KEY environment variable)"
echo "  2. Run: mymistral -m mistral-small-latest"
echo ""
